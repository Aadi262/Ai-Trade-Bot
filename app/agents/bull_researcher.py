...
Construct the STRONGEST possible BUY argument from the data provided.
Output ONLY JSON: {"signal": "BUY", "confidence": float, "reasoning": "string", "key_factors": [], "risk_flags": []}"""
...
class BullResearcher(BaseAgent):
    ...
    async def analyze(self, symbol: str, context: dict) -> AgentOutput:
        ...
        raw_text, _ = self._call_claude(SYSTEM_PROMPT, f"Build the bull case for {symbol}. Context:\n{json.dumps(context, default=str)}")
        try:
